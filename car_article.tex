...
\\
Another simple, secure and efficient code-based key exchange protocol is \textsc{Ouroboros} \cite{OUR}. The encryption algorithm uses Quasi-Cyclic Codes in Hamming metric, efficient decoding is achieved through BitFlipping. Encryption and decryption are faster than RSA for comparative benchmarks (https://bench.cr.yp.to). Ouroboros' integration into the OpenSSL/TLS library is planned and it is proposed as post-quantum secure algorithm at the NIST.
...
\Todo{Good Idea as outlook!} One of the latest and most challenging pq crypto ideas is the application of Isogeny based encryption schemes. There are alread first applications, as e.g. Microsofts VPN-client. \todo{cite openvpn from MS}
...
\bibitem
{SPH}
D.~J.~Bernstein, D.~Hopwood, A.~Hülsing, T.~Lange, R.~Niederhagen, L.~Papachristodoulou, M.~Schneider, P.~Schwabe, Z.~ Wilcox-O’Hearn.
